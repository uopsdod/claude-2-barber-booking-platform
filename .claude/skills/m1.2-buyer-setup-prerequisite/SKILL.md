...
> **Source of truth = GitHub `main` of the APP repo + the Supabase project — and in Cowork there is NO local checkout.** Don't "`git pull` your local" — the mounted working folder isn't a git repo. Instead **resolve the app repo (Step 0 below), then `git clone` it fresh from `main`** each session and **`git log -1`** to confirm the clone matches the deployed commit. A stale assumption about "the local code" wastes a session debugging "my code doesn't match the live app" ([[supabase-best-practice]] watch-out #11).

> **Step 0 — Resolve the APP repo (don't hardcode it; it's a DIFFERENT repo from the skills repo).** There are two repos: the **course/skills repo** (README + `.claude/skills` only, no app code) and the **app repo** (Vite/React, what Vercel deploys — this run it was `barberly-landing-auth`, but the name **differs per project**). **Discover the app repo, don't assume its name:** read it from the **Vercel deployment metadata** — `get_deployment → meta.githubRepo` / `meta.githubOrg` — or **reuse the URL M1.1's prerequisite already captured**. Echo it back and store it so `[[m1.2-buyer-setup]]` uses the same one (every "the repo" in the build skill means **this** app repo). If you clone the skills repo to "build the buyer pages", you'll find nothing to edit.
...
**3b — the M0 sign-up can mint a customer (a MECHANISM check — a customer row does NOT and should NOT exist yet).** On the student's own Supabase there is **only the one Shop account** from M1.1. The customer accounts M0 tested were signed up against **Lovable's own Supabase (Lovable Cloud)** — M0 then swapped auth onto the student's own project, so those Lovable-Cloud accounts **did not carry over**. So `select … where role='customer'` returning **zero rows is the correct, expected state** — not a gap to fill. All you confirm here is that the M0 sign-up (a customer/shop **toggle or tab**, whatever this build shipped — don't assume its exact UI) writes `role='customer'` correctly, so that when the build starts you *can* sign one up. Read the `profiles.role` allowlist (the same constraint M1.1's prereq checked), **not** a customer row:
```sql
