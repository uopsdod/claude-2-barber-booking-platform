...
> **Source of truth = GitHub `main` + the Supabase project, not a local checkout.** Before anything, have the student **`git pull` on `main`** and confirm the migration list is current — this milestone especially bites here: the DB/deploy can be at M2.2 (with the `payouts` table + admin policy) while a local `src/` is still at M2.1 (no `/admin/payouts`), and you'd debug the wrong layer ([[supabase-best-practice]] watch-out #11).
