...
Ask the student for: their **custom domain / host** (`book.yourdomain.com`), the **Route 53 `HostedZoneId`** (or auto-detect via `list-hosted-zones`), and **whether Stripe is already live**. Then run each section and report.
...
- **A1** The record set for the host exists and matches what Vercel asked for in [[m3-domain]] Step 1:
  ...
  aws route53 list-resource-record-sets --hosted-zone-id <zone-id> \
    --query "ResourceRecordSets[?Name=='book.yourdomain.com.']"
  ```
...
- **B2** Deep links resolve on the new host (no SSR-vs-SPA 404):
  ```bash
  curl -sS -o /dev/null -w "%{http_code}\n" https://book.yourdomain.com/login     # 200
  curl -sS -o /dev/null -w "%{http_code}\n" https://book.yourdomain.com/barbers   # 200 or redirect to /login
  ```
- **B3** **Auth + booking still work on the custom domain (the decisive test):** on `https://book.yourdomain.com`, sign in, open a barber's page (`/barbers/[id]`), open the **booking pop-up**, pick a slot, and confirm the flow runs (Checkout opens if Stripe is wired). This proves the *same* app works at the *new* address — not just that the host serves a 200. *Recovery:* if auth fails on the custom domain but worked on `*.vercel.app`, check the Supabase **redirect/allowed URLs** include the new host.

### Section C — Stripe webhook points at the new domain (only if Stripe is live)
- **C1** *(skip if Stripe isn't live)* In **Stripe → Developers → Webhooks**, the endpoint URL is `https://book.yourdomain.com/api/stripe/webhook` (path unchanged), and a test payment flips the **booking** `pending_payment → paid`. *Recovery:* update the endpoint URL in the Stripe dashboard ([[m3-domain]] Step 4) — the Stripe MCP doesn't manage webhook endpoints. The webhook is the source of truth for recording a booking as paid, so a stale/404 URL = payments succeed but bookings never flip to `paid`. (Full sandbox→live switch: [[stripe-go-live]].)

## Reporting

| Check | Status | Notes |
|---|---|---|
| A1 Route 53 record matches Vercel value | ✅ / ⚠️ / ❌ | CNAME (subdomain) or A/ALIAS (apex), exact target |
| B1 custom domain serves HTTPS 200 | ✅ / ❌ | valid cert |
| B2 deep links (/login, /barbers) resolve | ✅ / ❌ | no SSR-vs-SPA 404 |
| B3 auth + booking work on the custom domain | ✅ / ❌ | the decisive test |
| C1 Stripe webhook → new domain | ✅ / ⚠️ / ❌ | ⚠️ = Stripe not live (N/A) |

**Verdict:**
- All ✅ (C1 may be N/A if Stripe isn't live yet) → 「🎉 M3 完成！你的理髮預約平台已經掛在自己的網域 `book.yourdomain.com` 上、HTTPS 正常、登入和預約都跑得通 — 整套抽成制預約 SaaS 正式上線了。要開始收真錢的話，下一步是 `stripe-go-live`：換 live 金鑰、把正式 webhook 指到新網域、用真卡刷一筆再退一筆驗證。」
- Any ❌ → fix and re-run `上線檢查`: A1 mismatch → `UPSERT` the exact Vercel target; B1 not 200 → wait for DNS propagation / re-check the record + Vercel's green ✓; B3 auth fails → add the new host to Supabase allowed-redirect URLs; C1 wrong → repoint the Stripe webhook endpoint in the dashboard.
