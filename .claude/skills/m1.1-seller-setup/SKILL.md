...
> 4. **登入後要「依角色導向」，不要一律丟到同一頁——但 M1.1 只分流 `shop` 和 `customer` 兩種角色。** 登入成功後讀 `profiles.role`，再導向**「這個 build 裡實際存在的」對應頁面**：`shop` → 店家後台（此 build 可能叫 `/shop`、`/shop/bookings` 或別的），`customer` → 顧客落地頁（可能叫 `/barbers`、`/app` 或別的）。**⚠️ M1.1 不要寫任何 `admin` 分支，也不要出現 `/admin/payouts` 這個字串。** 原因：M1.1 根本產生不出一個登入中的 admin（`admin` 帳號要到 M2.1 前置才會用一次性 migration 升級出來，註冊/升級路徑只寫 `customer | shop`），所以 admin 這條分支是**永遠走不到的死碼**——而一旦寫死 `case 'admin': return '/admin/payouts'`，就等於埋了一顆 404 地雷：它在 M1.1 完全靜默（沒有 admin 觸發），卻會在 M2.1 升級出第一個 admin、對方一登入時才引爆。admin 的登入分支在 **M2.1** 才引入、真正的目標頁 `/admin/payouts` 在 **M2.2** 才接上；M1.1 這裡完全不碰。**用這個 build 的真實 route 名稱，不要照抄字面上的 `/barbers`**（M0 的 app 有可能用 `/app` 之類的名字）。註冊信的 `emailRedirectTo` 也用同一套邏輯。重點是「依 `profiles.role` 分流成 shop/customer」，路徑名稱以實際 app 為準。
>
...
- **M0's post-login route may not be `/barbers`.** The M0 app may route to `/app` (or another name). **Make the redirect route-NAME-agnostic:** read `profiles.role` once and branch to *whatever this build's* customer-landing / shop-dashboard page actually is — don't hardcode literal `/barbers`. **M1.1's redirect branches `shop` and `customer` ONLY — do NOT add an `admin` branch, and do NOT put the string `/admin/payouts` anywhere.** There is no logged-in admin in M1.1 (admin is first promoted in the M2.1 prereq; the sign-up/upgrade path only ever writes `customer | shop`), so an `admin` branch is unreachable dead code — and writing `case 'admin': return '/admin/payouts'` plants a **404 landmine**: silent through all of M1.1 (nothing triggers it), then detonating the moment M2.1 promotes the first admin and they log in. The admin branch is **introduced in M2.1** (when the first admin exists) and its real target `/admin/payouts` is **wired in M2.2** (when the page is built) — M1.1 leaves it out entirely. The load-bearing rule is "**branch `profiles.role` into shop/customer**", not the specific path strings.

A blanket redirect to one page strands the wrong roles: a shop lands on the customer page instead of its dashboard. Reading `profiles.role` once after auth and branching `shop`/`customer` to the app's real routes fixes it at the source. (Admin settlement access is an M2.1/M2.2 concern — deliberately not part of M1.1's redirect; the admin *role value* still exists in the schema/RLS here so M2.2's payout page can read shop bank fields, but no admin can log in yet.)
