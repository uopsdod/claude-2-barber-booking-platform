...
Takes the working booking platform (live on `*.vercel.app`) and puts it on **the student's own domain** — `book.yourdomain.com` instead of `barber-platform.vercel.app`. The student **already owns a domain**; M3 does **no registration**. The only moving part is **DNS**: Vercel tells you exactly which records to create, and you create them in **AWS Route 53** (the same AWS account you connected in M0 — no new auth).

By the end the student has:

1. The custom domain (a **subdomain** like `book.yourdomain.com` by default) **added to the Vercel project**, with Vercel showing the exact DNS records it needs.
2. Those **Vercel-provided records created in the Route 53 hosted zone** — a `CNAME` → `cname.vercel-dns.com` for a subdomain, or `A`/`ALIAS` records for an apex — via the AWS API MCP (`call_aws` → `route53 change-resource-record-sets`, always **`UPSERT`**).
3. Vercel showing the domain **Valid / green ✓** with an **automatic HTTPS cert** issued.
4. The live booking app served over **`https://book.yourdomain.com`** — landing, `/login`, `/barbers`, the booking pop-up — all working on the new host.
...
- **Product Site relabel `*.vercel.app` → `book.yourdomain.com`** (top): Step 1 adds the domain in Vercel; Step 2 makes DNS point at Vercel via Route 53; Step 3 turns it green + HTTPS. The hosting target does **not** move — it's the same Vercel deployment, just a new address.
...
- **Stripe webhook → `book.yourdomain.com`** (right): Step 4, *only if Stripe is already live* — repoint the webhook endpoint so confirmations keep landing ([[stripe-go-live]]).
...
1. Add the custom domain in Vercel → read off the DNS records Vercel shows
2. Create those records in the Route 53 hosted zone (`call_aws` → `UPSERT`)
3. Wait for Vercel to verify (green ✓) + the HTTPS cert
4. *(If Stripe is live)* update the Stripe webhook endpoint URL → then verify with the checklist

---

### Step 1 — Add the custom domain in Vercel and read off the records

> **Default to a SUBDOMAIN, not the apex — and don't ask.** Bind **`book.yourdomain.com`** by default, even when the apex is free. A subdomain takes a single clean `CNAME`, never collides with anything else the student has on that domain, and avoids the apex-`A`/`ALIAS` special-casing. Only use the bare apex (`yourdomain.com`) if the student explicitly asks for it.
...
> 到 **Vercel → 你的 project → Settings → Domains → Add**，輸入 **`book.yourdomain.com`**（用 `book.` 子網域，最單純）→ Add。

**CLI equivalent:**
```bash
vercel domains add book.yourdomain.com
```
...
- **Subdomain (default)** → a single **`CNAME`**: `book.yourdomain.com` → **`cname.vercel-dns.com`**.
...
> Have the student **paste the exact host + record type + target Vercel shows** back to you. That string is the source of truth for Step 2.
...
> 回到 **Vercel → Settings → Domains**，等 `book.yourdomain.com` 從「Pending / Invalid Configuration」變成 **綠色的 ✓ Valid**，憑證會自動簽發。DNS 傳播可能要幾分鐘 — 沒馬上變綠很正常，等一下再看。

Confirm the live host actually serves the app over HTTPS:
```bash
# CLI mode:
curl -sSI https://book.yourdomain.com | head -1     # HTTP/2 200
```
...
> 到 **Stripe Dashboard → Developers → Webhooks → 你的 endpoint → Update details**，把 endpoint URL 從 `https://<app>.vercel.app/api/stripe/webhook` 改成 `https://book.yourdomain.com/api/stripe/webhook`（路徑保持一致）。改完做一筆測試付款，確認 booking 還是會被 webhook 設成 `paid`。
...
**Setup is done. Run [[m3-domain-checklist]]** (the student can say 「驗收 M3」/「上線檢查」) to confirm the record matches Vercel, the domain serves HTTPS, auth + booking still work on the new host, and the Stripe webhook (if live) points at the new domain.

---

## Things to watch out for (common mistakes)

1. **Default to a subdomain — don't ask.** Bind `book.yourdomain.com` by default, even on a fresh domain. It's a single clean `CNAME`, never collides with the student's other records, and skips the apex `A`/`ALIAS` special-casing. Bare apex only if the student explicitly asks.
...
When `m3-domain-checklist` is green, tell the student (milestone-scoped — celebrate M3, don't surface the next step):
「M3 完成了！你的理髮預約平台現在掛在你自己的網域 `book.yourdomain.com` 上，HTTPS 憑證自動簽好，登入和預約都能用 — 整套抽成制預約 SaaS 從 landing、開店排程、線上付款、到月底撥款後台都跑通了，恭喜上線 🎉」

> **Note for Claude Code (author-only — do NOT volunteer to the student):** the optional follow-on is taking Stripe from test mode to live (swap to `sk_live_`/`pk_live_` keys, point a live webhook at `book.yourdomain.com`, smoke-test with a real card + refund). Only bring it up if the student asks about charging real money; then load `[[stripe-go-live]]`. Don't seed go-live into the student's mental model at the end of M3.
